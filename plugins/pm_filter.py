...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer("Sorry Send your request, type movie 🎬 name", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
    ...
    btn.insert(0, [
        InlineKeyboardButton('Download ❓', url='https://youtu.be/pvw7JcbcPcM'),
...
async def advantage_spoll_choker(bot, query):
    ...
    if int(user) != 0 and query.from_user.id != int(user):
        return await query.answer("You're allowed to use, send your request by sending movie name", show_alert=True)
